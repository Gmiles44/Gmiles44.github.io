...
@app.route("/")
def index():
    stats = db.execute("SELECT * FROM stats")[0]
...
def craft():
    recipes = db.execute("""SELECT c.name, c.base_price, ci.amount AS owned, m1.name AS m1_name, m1_amt, i1.amount AS i1_amt,
                            ...
                            m3.name AS m3_name, m3_amt, i3.amount AS i3_amt, c.id, level
                            FROM recipes
                            ...
                            LEFT JOIN creations_inventory ci ON recipes.creation_id = ci.creation_id ORDER BY c.base_price""")
    stats = db.execute("SELECT * FROM stats")[0]
    if request.method == "GET":
    ...
    if request.method == "POST":
        ...
        if int(db.execute("SELECT energy FROM recipes WHERE creation_id = ?", id)[0]['energy']) > int(db.execute("SELECT energy FROM stats WHERE player_id = 1")[0]['energy']):
        ...
        def craft_object():
            ...
            if len(db.execute("SELECT * FROM creations_inventory WHERE creation_id = ?", id)) == 0:
                db.execute("INSERT INTO creations_inventory (creation_id, amount) VALUES (?, 1)", id)
            else:
                db.execute("UPDATE creations_inventory SET amount = amount + 1 WHERE creation_id = ?", id)
            db.execute("UPDATE stats SET exp = exp + (SELECT exp FROM recipes WHERE creation_id = ?) WHERE 1", id)
            db.execute("UPDATE stats SET energy = energy - (SELECT energy FROM recipes WHERE creation_id = ?) WHERE 1", id)
...
def market():
    ...
    inventory = db.execute("""SELECT i.amount, c.name, c.base_price, c.id FROM creations_inventory i
                            JOIN creations c ON c.id = i.creation_id""")
    stats = db.execute("SELECT * FROM stats")[0]
    ...
    if request.method == "POST":
        ...
        gold = int(db.execute("SELECT gold FROM stats")[0]['gold'])
        if gold < total:
            flash("Not enough Gold!")
            return redirect("/market")
        elif len(db.execute("SELECT * FROM materials_inventory WHERE mat_id = ?", id)) == 0:
            db.execute("INSERT INTO materials_inventory (mat_id, amount) VALUES (?, ?)", id, amount)
        else:
            db.execute("UPDATE materials_inventory SET amount = amount + ? WHERE mat_id = ?", amount, id)
        db.execute("UPDATE merchants SET inv_?_amt = inv_?_amt - ? WHERE id = ?", m_number, m_number, amount, merch_id)
        db.execute("UPDATE stats SET gold = gold - ?", total)
...
def sell():
    ...
    db.execute("UPDATE stats SET gold = gold + ?", total)
    db.execute("UPDATE creations_inventory SET amount = amount - ? WHERE creation_id = ?", amount, id)
...
def end_day():
    stats = db.execute("SELECT * FROM stats WHERE player_id = 1")[0]
    merchant_ids = db.execute("SELECT id FROM merchants")
    for id in merchant_ids:
        db.execute("""UPDATE merchants SET
                inv_1_amt = (SELECT ABS(RANDOM() % 7) + 5),
                inv_2_amt = (SELECT ABS(RANDOM() % 5) + 4),
                inv_3_amt = (SELECT ABS(RANDOM() % 4) + 3),
                inv_4_amt = (SELECT ABS(RANDOM() % 4) + 1),
                inv_5_amt = (SELECT ABS(RANDOM() % 3))
                WHERE id = ?""", id['id']
        )
    if stats['exp'] >= stats['next_level']:
        db.execute("UPDATE stats SET level = level + 1 WHERE player_id = 1")
        db.execute("""UPDATE stats SET max_energy =
                   (SELECT max_energy FROM levels WHERE level =
                   (SELECT level FROM stats WHERE player_id = 1)),
                   next_level = (SELECT next_level FROM levels WHERE level = (
                   SELECT level FROM stats WHERE player_id = 1)) WHERE player_id = 1""")
        db.execute("UPDATE stats SET exp = 0 WHERE player_id = 1")
...
def tavern():
    ...
    if request.method == "GET":
        stats = db.execute("SELECT * FROM stats WHERE 1")[0]
    ...
    if request.method == "POST":
        energy = db.execute("SELECT energy FROM stats WHERE player_id = 1")[0]['energy']
        max_energy = db.execute("SELECT max_energy FROM stats WHERE player_id = 1")[0]['max_energy']
        gold = db.execute("SELECT gold FROM stats WHERE player_id = 1")[0]['gold']
        ...
        if request.form.get("rent"):
            ...
            if energy >= max_energy:
                ...
                return redirect("/tavern")
            db.execute("UPDATE stats SET gold = gold - (SELECT price FROM lodging WHERE id = ?) WHERE player_id = 1", id)
            db.execute("UPDATE stats SET energy = energy + ROUND(max_energy * (SELECT recovery / 100 FROM lodging WHERE id = ?)) WHERE player_id = 1", id)
            if energy > max_energy:
            ...
            if energy >= max_energy:
                ...
                return redirect("/tavern")
            db.execute("UPDATE stats SET gold = gold - (SELECT price FROM food WHERE id = ?) WHERE player_id = 1", id)
            db.execute("UPDATE stats SET energy = energy + (SELECT energy FROM food WHERE id = ?) WHERE player_id = 1", id)
            energy = db.execute("SELECT energy FROM stats WHERE player_id = 1")[0]['energy']
            if energy > max_energy:
                db.execute("UPDATE stats set energy = max_energy WHERE player_id = 1")
            return redirect("/tavern")
